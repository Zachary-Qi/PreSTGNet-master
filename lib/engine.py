...
from modules.lsfgcn_arch.mask_pretrain import Mask  
from modules.lsfgcn_arch.lsst_fusion import STGNet 
...
def setup_distributed_backend_run(args, local_rank):
    ...
    if args.mode == "PreTrain":
        ...
        predict_model = STGNet(args.history_seq_len, args.future_seq_len, args.in_channel_train, args.num_of_vertices, 
...
class trainer:
    ...
    def train(self, dataCollections):
        ...
        if self.mode == "Train":
            ...
            output, reg_loss = self.predict_model(input, hidden_states_full)
            
            predict = re_standard_transform(output, self.scaler)
            loss = self.loss(predict, real) + reg_loss
            ...
            self.model.train()
            self.optimizer.zero_grad()
            output_x, output_y, reg_loss = self.model(input)
            true_loss = self.loss(output_x, output_y) + reg_loss
    ...
    def eval(self, dataCollections):
        ...
        if self.mode == "Train":
            ...
            with torch.no_grad():
                hidden_states_full = self.model(input_z)
                output,_ = self.predict_model(input, hidden_states_full)
                predict = re_standard_transform(output, self.scaler)
                loss = self.loss(predict, real)
        else:
            input = dataCollections.cuda()
            ...
            self.model.eval()
            with torch.no_grad():
                output_x, output_y,_ = self.model(input)
