...
from lib.utils_test import *


DATASET_NAME = "PeMS08"
MODEL_NAME = "PreSTGNet"
MODE = "Train" # only Train or PreTrain
PRE_TRAINED_WEIGHT_PATH = "./experiments/PeMS08_PreSTGNet/PreTrain/2026-03-08_08-15/session_1/checkpoint.pth"
Breakpoint_Destination_Folder = ""
...
def trainer_distributed(args, local_rank, world_size, train_loader, valid_loader, test_loader, scaler, logger):
    ...
    if args.mode == "PreTrain": 
        ...
        if local_rank == 0:
            ...
            prediction_true_name = "./datasets/visualData/{}_{}_prediction.h5".format("My", DATASET_NAME)      
            target_true_name = "./datasets/visualData/{}_{}_target.h5".format("My", DATASET_NAME)   
            print(f"predicts_all:{predicts_all.shape}, reals_all:{reals_all.shape}")
            save_dir = os.path.dirname(prediction_true_name)
            os.makedirs(save_dir, exist_ok=True)
            
            with h5py.File(prediction_true_name, 'w') as f:
                f.create_dataset('tensor', data=predicts_all.cpu().numpy())
            
            with h5py.File(target_true_name, 'w') as f:
                f.create_dataset('tensor', data=reals_all.cpu().numpy())
